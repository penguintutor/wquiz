#!/usr/bin/perl -w
# Offline quiz - choose quiz - using list
...
my $template = "templates/offline2.html";
my $page = "offline2.pl";
...
for ($i=0; $i < scalar @category; $i++)
	...
	$optionmenu .= "<li><a href=\"offlinequestions.pl?quiz=$category[$i]&amp;style=$style\" accesskey=\"".($i+1)."?new=1\">".$quiznames{$category[$i]}."</a></li>";
	}
